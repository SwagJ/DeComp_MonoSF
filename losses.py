...
from utils.sceneflow_util import pixel2pts_ms, pts2pixel_ms, reconstructImg, reconstructPts, projectSceneFlow2Flow, flow2sf_dispC, flow2sf_dispC_v2, flow2sf_dispC_formula2, flow2sf_depthC, flow2sf_dispC_validate
...
from models.modules_sceneflow import WarpingLayer_Flow, get_grid_exp, WarpingLayer_Flow_W_Mask
...
class Eval_SceneFlow_KITTI_Train_Param(nn.Module):
	...
	def forward(self, output_dict, target_dict):
		...
		out_depth_l1 = torch.clamp(out_depth_l1, 1e-3, 80)
		gt_depth_l1 = _disp2depth_kitti_K(gt_disp, intrinsics[:, 0, 0])
		gt_depth_l2 = _disp2depth_kitti_K(gt_disp2_occ, intrinsics[:,0,0])
		...
		gt_pts1, _ = pixel2pts(intrinsics, gt_depth_l1)
		gt_pts2, _ = pixel2pts(intrinsics, gt_depth_l2)
		warpped_gt_pts2 = self.warping_layer(gt_pts2 * gt_disp2_mask, gt_flow) * gt_flow_mask
		gt_sf = warpped_gt_pts2 - gt_pts1 * gt_disp_mask
		sf_err = _elementwise_epe(out_sceneflow, gt_sf) * gt_sf_mask
		sf_err_np = sf_err.cpu().detach().squeeze(0).numpy().transpose(1,2,0)
		loss_dict['sf_epe'] = (sf_err.view(batch_size, -1).sum(1)).mean() / 91875.68

		sf_err_color = cv2.applyColorMap(cv2.convertScaleAbs(sf_err_np, alpha=255/sf_err_np.max()), cv2.COLORMAP_JET)
		#print(sf_err_color.shape)
		#error_flow_name = save_dir_flow + '/' + str(i).zfill(6) + '.png'
		sf_err_path = self.args.save + '/sf_err/'
		if not os.path.isdir(sf_err_path):
			os.makedirs(sf_err_path)
		cv2.imwrite(sf_err_path + str(target_dict['basename'][0]) + '.png' , sf_err_color)

		# formulation 2:
		sf_2 = flow2sf_dispC_validate(gt_flow , gt_depth_l1 , gt_depth_l2, intrinsics)
		flow_2 = projectSceneFlow2Flow(target_dict['input_k_l1'], sf_2, gt_disp)

		sf_err_2 = _elementwise_epe(sf_2, gt_sf) * gt_sf_mask
		sf_err_2_np = sf_err_2.cpu().detach().squeeze(0).numpy().transpose(1,2,0)
		loss_dict['sf_2_epe'] = (sf_err_2.view(batch_size, -1).sum(1)).mean() / 91875.68
		sf_2_err_color = cv2.applyColorMap(cv2.convertScaleAbs(sf_err_2_np, alpha=255/sf_err_2.max()), cv2.COLORMAP_JET)

		sf_err_2_path = self.args.save + '/sf_2_err/'
		if not os.path.isdir(sf_err_2_path):
			os.makedirs(sf_err_2_path)
		cv2.imwrite(sf_err_2_path + str(target_dict['basename'][0]) + '.png' , sf_2_err_color)

		flow_err_2 = _elementwise_epe(flow_2, gt_flow) * gt_flow_mask
		flow_err_2_np = flow_err_2.cpu().detach().squeeze(0).numpy().transpose(1,2,0)
		loss_dict['flow_epe'] = (flow_err_2.view(batch_size, -1).sum(1)).mean() / 91875.68
		flow_err_2_color = cv2.applyColorMap(cv2.convertScaleAbs(flow_err_2_np, alpha=255/flow_err_2_np.max()), cv2.COLORMAP_JET)

		flow_err_2_path = self.args.save + '/flow_err_2/'
		if not os.path.isdir(flow_err_2_path):
			os.makedirs(flow_err_2_path)
		cv2.imwrite(flow_err_2_path + str(target_dict['basename'][0]) + '.png' , flow_err_2_color)
...
class Eval_MonoFlowDispExp_KITTI_Train(nn.Module):
	...
	def forward(self, output_dict, target_dict):
		...
		disp_l1_next = output_dict["disp_l1_pp"][0] / torch.exp(output_dict["exp_f_pp"][0])
...
class Eval_MonoFlowDispC_KITTI_Train(nn.Module):
	...
	def forward(self, output_dict, target_dict):
		...
		## Depth 2
		##################################################
		dispC = interpolate2d_as(output_dict["dispC_f_pp"][0], gt_disp, mode="bilinear")
		disp_l1_next = out_disp_l1 + dispC
		#disp_l1_next = interpolate2d_as(disp_l1_next, gt_disp, mode="bilinear") * width
...
class Eval_MonoDispC_KITTI_Train(nn.Module):
	def __init__(self, args):
		super(Eval_MonoDispC_KITTI_Train, self).__init__()
	...
	def upsample_dispC_as(self, dispC, output_as):
		size_inputs = dispC_b.size()[2:4]
		size_targets = output_as.size()[2:4]
		resized_flow = tf.interpolate(dispC, size=size_targets, mode="bilinear", align_corners=True)
		# correct scaling of flow
		#u, v = resized_flow.chunk(2, dim=1)
		resized_flow *= float(size_targets[1] / size_inputs[1])
		#v *= float(size_targets[0] / size_inputs[0])
		return resized_flow
	...
	def forward(self, output_dict, target_dict):
		...
		batch_size, _, _, width = gt_disp.size()
		#print(output_dict["disp_l1_pp"][-1].shape)
		out_disp_l1 = interpolate2d_as(output_dict["disp_l1"], gt_disp, mode="bilinear") * width
		#out_disp_l1 = target_dict['disp_pre'].cuda()
		out_depth_l1 = _disp2depth_kitti_K(out_disp_l1, intrinsics[:, 0, 0])
		out_depth_l1 = torch.clamp(out_depth_l1, 1e-3, 80)
		gt_depth_l1 = _disp2depth_kitti_K(gt_disp, intrinsics[:, 0, 0])
		...
		##################################################
		#print(output_dict['flow_f_pp'][0])
		#print(output_dict['scaled_flow_f'])
		...
		dispC = interpolate2d_as(output_dict['dchange_f'], gt_disp, mode = "bilinear")
		disp_l1_next = out_disp_l1 + dispC
		#disp_l1_next = interpolate2d_as(disp_l1_next, gt_disp, mode="bilinear")
		#disp_l1_next = interpolate2d_as(disp_l1_next, gt_disp, mode="bilinear") * width
...
class Eval_MonoFlowDepthC_KITTI_Train(nn.Module):
	def __init__(self, args):
		super(Eval_MonoFlowDepthC_KITTI_Train, self).__init__()
	...
	def forward(self, output_dict, target_dict):
		...
		depth_change = interpolate2d_as(output_dict["dispC_f_pp"][0], gt_disp, mode="bilinear")
		out_depth_l1_next = out_depth_l1 + depth_change

		disp_l1_next = _depth2disp_kitti_K(out_depth_l1_next, intrinsics[:, 0, 0])
		#out_disp_l1_next = _depth2disp_kitti_K(out_depth_l1_next, intrinsics[:, 0, 0])
		gt_depth_l1_next = _disp2depth_kitti_K(gt_disp2_occ, intrinsics[:, 0, 0])
...
class Loss_FlowDisp_SelfSup_CensusFlow_SSIM_Disp_Top_Consistency(nn.Module):
	def __init__(self, args):
		super(Loss_FlowDisp_SelfSup_CensusFlow_SSIM_Disp_Top_Consistency, self).__init__()
		...
		self._sf_3d_sm = 200
		self._warping_layer = WarpingLayer_Flow()
		self._warping_layer_consistency = WarpingLayer_Flow_W_Mask()
	...
	def depth_loss_left_img_l2(self, disp_l, disp_r, img_l_aug, img_r_aug, weight):
		...
		## Photometric loss
		img_diff= (_elementwise_l1(img_l_aug, img_r_warp) * (1.0 - self._ssim_w) + _SSIM(img_l_aug, img_r_warp) * self._ssim_w).mean(dim=1, keepdim=True)
		img_diff= img_diff * weight       
		loss_img = (img_diff[left_occ]).mean()
		img_diff[~left_occ].detach_()

		## Disparities smoothness
		loss_smooth = _smoothness_motion_2nd(disp_l, img_l_aug, beta=10.0).mean() 
		#print(torch.isnan(loss_img), torch.isnan(loss_smooth))

		return loss_img + self._disp_smooth_w * loss_smooth, left_occ

	def consistency_loss(self, disp_l1, disp_l2, flow_f):
		disp_l2_warp, mask = self._warping_layer_consistency(disp_l2, flow_f)
		diff = _elementwise_l1(disp_l1, disp_l2_warp) / (disp_l1 + disp_l2_warp)
		loss_consist = (diff * mask).mean()
		weight = 1 - (diff / diff.max() * mask)

		return loss_consist, weight
	...
	def forward(self, output_dict, target_dict):
		...
		## Disp Loss
		loss_disp_l1, disp_occ_l1 = self.depth_loss_left_img(disp_l1, disp_r1, img_l1_aug, img_r1_aug, 0)
		loss_consist, weight = self.consistency_loss(disp_l1, disp_l2, sf_f)
		loss_disp_l2, disp_occ_l2 = self.depth_loss_left_img_l2(disp_l2, disp_r2, img_l2_aug, img_r2_aug, weight)
		loss_dp_sum = loss_dp_sum + (loss_disp_l1 + loss_disp_l2) + 0.5 * loss_consist
...
class Loss_MonoExp_SelfSup_v2(nn.Module):
	def __init__(self, args):
		super(Loss_MonoExp_SelfSup_v2, self).__init__()
	...
	def forward(self, output_dict, target_dict):
		...
		sf_f = self.upsample_flow_as(output_dict['scaled_flow_f'], target_dict['input_l1'])
		sf_b = self.upsample_flow_as(output_dict['scaled_flow_b'], target_dict['input_l1'])
		exp_f = output_dict['dchange_f']
		exp_b = output_dict['dchange_b']
		mask_f = output_dict['mask_f'].unsqueeze(1)
		mask_b = output_dict['mask_b'].unsqueeze(1)
...
class Loss_MonoDispC_SelfSup(nn.Module):
	def __init__(self, args):
		super(Loss_MonoDispC_SelfSup, self).__init__()
	...
	def sceneflow_loss(self, exp_f, exp_b, flow_f, flow_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
		...
		sf_f = flow2sf_dispC_formula2(flow_f, disp_l1, exp_f, k_l1_aug, local_scale / aug_size)
		sf_b = flow2sf_dispC_formula2(flow_b, disp_l2, exp_b, k_l2_aug, local_scale / aug_size) 
...
class Loss_MonoFlowDisp_DispC_Sceneflow_v2(nn.Module):
	def __init__(self, args):
		super(Loss_MonoFlowDisp_DispC_Sceneflow_v2, self).__init__()
	...
	def sceneflow_loss(self, dispC_f, dispC_b, flow_f, flow_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
		...
		sf_f = flow2sf_dispC_formula2(flow_f, disp_l1, dispC_f, k_l1_aug, local_scale / aug_size)
		sf_b = flow2sf_dispC_formula2(flow_b, disp_l2, dispC_b, k_l2_aug, local_scale / aug_size) 
		...
		pts2_warp = self._warping_layer(pts2, flow_f)
		pts1_warp = self._warping_layer(pts1, flow_b)
		...
		## Loss Summnation
		sceneflow_loss = self._sf_3d_pts * loss_pts + loss_dispC_s * 0.1
...
class Loss_MonoFlowDisp_DispC_Sceneflow_v2_Joint(nn.Module):
	def __init__(self, args):
		super(Loss_MonoFlowDisp_DispC_Sceneflow_v2_Joint, self).__init__()
	...
	def sceneflow_loss(self, dispC_f, dispC_b, flow_f, flow_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
		...
		## Loss Summnation
		sceneflow_loss = loss_im + self._sf_3d_pts* loss_pts / 4 + loss_dispC_s * 0.1
...
class Loss_MonoFlowDisp_DispC_Sceneflow_v2_Proj(nn.Module):
	def __init__(self, args):
		super(Loss_MonoFlowDisp_DispC_Sceneflow_v2_Proj, self).__init__()
	...
	def sceneflow_loss(self, dispC_f, dispC_b, flow_f, flow_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
		...
		flow_f_proj = projectSceneFlow2Flow(k1_scale, sf_f, disp_l1)
		flow_b_proj = projectSceneFlow2Flow(k2_scale, sf_b, disp_l2)

		occ_map_f = _adaptive_disocc_detection(flow_f_proj).detach()
		occ_map_b = _adaptive_disocc_detection(flow_b_proj).detach()
		...
		## Loss Summnation
		sceneflow_loss = loss_im + self._sf_3d_pts * loss_pts + self._disp_smooth_w / 2 * loss_dispC_s
...
class Loss_MonoFlowDisp_DispC_Sceneflow_v2_Consistency(nn.Module):
	def __init__(self, args):
		super(Loss_MonoFlowDisp_DispC_Sceneflow_v2_Consistency, self).__init__()
	...
	def sceneflow_loss(self, dispC_f, dispC_b, flow_f, flow_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
		...
		# flow_reg_loss
		loss_flow_f = _elementwise_epe(flow_f,flow_f_proj)
		loss_flow_b = _elementwise_epe(flow_b,flow_b_proj)
		...
		loss_disp_f = _elementwise_l1(disp_l1 + dispC_f, disp_l2)
		loss_disp_b = _elementwise_l1(disp_l2 + dispC_b, disp_l1)
		loss_d_f = loss_disp_f[disp_occ_l1].mean()
		loss_d_b = loss_disp_b[disp_occ_l2].mean()
		loss_disp_f[~disp_occ_l1].detach_()
		loss_disp_b[~disp_occ_l2].detach_()
		loss_d = loss_d_f + loss_d_b
		#print("loss_pts dim", loss_pts.shape)
		...
		## Loss Summnation
		sceneflow_loss = loss_flo + self._sf_3d_pts * loss_d + self._disp_smooth_w * loss_dispC_s
		
		return sceneflow_loss, loss_flo, loss_d, loss_dispC_s
	...
	def forward(self, output_dict, target_dict):
		...
		loss_dict = {}
		loss_dict["sf"] = loss_sf_sum
		loss_dict["flo"] = loss_sf_2d
		loss_dict["disp"] = loss_sf_3d
		loss_dict["s_3s"] = loss_sf_3d_sm
		loss_dict["total_loss"] = total_loss
...
class Loss_MonoFlowDisp_DispC_Sceneflow_v2_Consistency_v2(nn.Module):
	def __init__(self, args):
		super(Loss_MonoFlowDisp_DispC_Sceneflow_v2_Consistency_v2, self).__init__()
	...
	def sceneflow_loss(self, dispC_f, dispC_b, flow_f, flow_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
		...
		disp_l2_warp, mask_f = self._warping_layer_consistency(disp_l2, flow_f)
		loss_disp_f = _elementwise_l1(disp_l1 + dispC_f, disp_l2_warp) / (disp_l1 + disp_l2_warp)

		disp_l1_warp, mask_b = self._warping_layer_consistency(disp_l1, flow_b)
		loss_disp_b = _elementwise_l1(disp_l2 + dispC_b, disp_l1_warp) / (disp_l2 + disp_l1_warp)

		mask_f = mask_f.bool() & disp_occ_l1
		mask_b = mask_b.bool() & disp_occ_l2
		loss_d_f = loss_disp_f[mask_f].mean()
		loss_d_b = loss_disp_b[mask_b].mean()
		loss_disp_f[~mask_f].detach_()
		loss_disp_b[~mask_b].detach_()
		loss_d = loss_d_f + loss_d_b
		#print("loss_pts dim", loss_pts.shape)
...
class Loss_MonoFlowDispDispC_SelfSup_v2(nn.Module):
	def __init__(self, args):
		super(Loss_MonoFlowDispDispC_SelfSup_v2, self).__init__()
	...
	def sceneflow_loss(self, dispC_f, dispC_b, flow_f, flow_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
		...
		## Image reconstruction loss
		img_l2_warp = self.warping_layer(img_l2_aug, flow_f)
		img_l1_warp = self.warping_layer(img_l1_aug, flow_b)
	...
	def forward(self, output_dict, target_dict):
		...
		for ii, (sf_f, sf_b, disp_l1, disp_l2, disp_r1, disp_r2, dispC_f, dispC_b) in enumerate(zip(output_dict['flow_f'], output_dict['flow_b'], output_dict['disp_l1'], output_dict['disp_l2'], disp_r1_dict, disp_r2_dict, output_dict['dispC_f'], output_dict['dispC_b'])):
